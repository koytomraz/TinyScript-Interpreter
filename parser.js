...
class Parser {
  ...
  _parseAssignment() {
    ...
    if (this._check(TokenType.ASSIGN)) {
	  ...
	  if (left.type === 'Identifier') return AST.AssignmentExpression(left.name, value);
	  if (left.type === 'IndexExpression') return AST.ArrayAssignment(left.array.name, left.indexes, value);
    }
